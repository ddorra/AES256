#ifndef QAESENCRYPTION_H
#define QAESENCRYPTION_H


#include <QObject>
#include <QByteArray>

class AES_Encryption : public QObject
{
    Q_OBJECT
public:
    enum Padding { ISO };

    static QByteArray Crypt(const QByteArray &rawText, const QByteArray &key,const QByteArray &iv = QByteArray(), AES_Encryption::Padding padding = AES_Encryption::ISO);
    static QByteArray Decrypt(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv = QByteArray(), AES_Encryption::Padding padding = AES_Encryption::ISO);
    static QByteArray keyExpansion(const QByteArray &key);
    static QByteArray RemovePadding(const QByteArray &rawText, AES_Encryption::Padding padding = AES_Encryption::ISO);

    AES_Encryption(AES_Encryption::Padding padding = AES_Encryption::ISO);

    QByteArray Encode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv = QByteArray());
    QByteArray Decode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv = QByteArray());
    QByteArray removePadding(const QByteArray &rawText);
    QByteArray KeyExpansion(const QByteArray &key);

    QByteArray printArray(uchar *arr, int size);
signals:

public slots:

private:
    int m_nb;
    int m_blocklen;
    int m_nk;
    int m_keyLen;
    int m_nr;
    int m_expandedKey;
    int m_padding;
    QByteArray* m_state;

    quint8 getSBoxValue(quint8 num){return sbox[num];}
    quint8 getSBoxInvert(quint8 num){return inv_sbox[num];}

    void addRoundKey(const quint8 round, const QByteArray &expKey);
    void sub_Bytes();
    void shiftRows();
    void mixColumns();
    void invMixColumns();
    void invSubBytes();
    void invShiftRows();
    QByteArray getPadding(int currSize, int alignment);
    QByteArray cipher(const QByteArray &expKey, const QByteArray &in);
    QByteArray invCipher(const QByteArray &expKey, const QByteArray &in);
    QByteArray byteXor(const QByteArray &a, const QByteArray &b);

    const quint8 sbox[256] = {
       };

    const quint8 inv_sbox[256] = {
       };

    const quint8 rcon[14] = {
        0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab};
};

#endif // AES_Encryption_H
